...
if __name__ == "__main__":
        ...
        for rpolicy_idx in range(4):
            ...
            for action in range(n_a):
                ...
                if action == 0:
                    ...
                    if B1_col < 0:
                        trans_string = f"{action}|{state_index}|{n_s-2}"
                        
                        if trans_string not in transition_values.keys():
                            transition_values[trans_string] = 0
                        
                        transition_values[trans_string] += b_move_prob
                        ...
                        trans_string1 = f"{action}|{state_index}|{new_state_index}"
                        trans_string2 = f"{action}|{state_index}|{n_s-2}"
                        
                        if trans_string1 not in transition_values.keys():
                            transition_values[trans_string1] = 0
                            
                        if trans_string2 not in transition_values.keys():
                            transition_values[trans_string2] = 0
                        
                        
                        if has_ball == 1:
                            if (B1_pos == R_pos) or (B1_prev_pos == R_pos and R_prev_pos == B1_pos):
                                transition_values[trans_string1] += b_move_prob * (0.5-p)
                                transition_values[trans_string2] += b_move_prob * (0.5+p)
                            else:
                                transition_values[trans_string1] += b_move_prob * (1-2*p)
                                transition_values[trans_string2] += b_move_prob * (2*p)
                        else:
                            transition_values[trans_string1] += b_move_prob * (1-p)
                            transition_values[trans_string2] += b_move_prob * (p)
                    ...
                    transition_values[trans_string1] += b_move_prob * passing_prob
                    transition_values[trans_string2] += b_move_prob * (1-passing_prob)
                    ...
                    trans_string1 = f"{action}|{state_index}|{n_s-2}"
                    trans_string2 = f"{action}|{state_index}|{n_s-1}"
                    ...
                    transition_values[trans_string1] += b_move_prob * (1-goal_prob)
                    transition_values[trans_string2] += b_move_prob * goal_prob
    ...
    for key, value in transition_values.items():
        ...
        print(f"transition {s1} {action} {s2} {reward} {value}")
